...
//**************URI ARAUTO ON***********************   
  server.on("/arAutoOn", []() 
  {
    Serial.println("Handler : Requisição Web /arAutoOn");
    arAUTO=true;
...
//**************URI ARAUTO OFF***********************   
  server.on("/arAutoOff", []() 
  {
    Serial.println("Handler : Requisição Web /arAutoOff");
    arAUTO=false;
...
//**************URI arTemp***********************   
  server.on("/arTemp", []() 
  {
    Serial.println("Handler : Requisição Web /arTemp");
...
//**************URI coolerTemp***********************   
  server.on("/coolerTemp", []() 
  {
    Serial.println("Handler : Requisição Web /coolerTemp");
    COOLER=true;
    if (server.method() != HTTP_POST) 
    {
      server.send(405, "text/plain", "Method Not Allowed");
    ...
    else 
    {
      ...
      temperaturaCOOLER = message.toInt();
      Serial.println(temperaturaCOOLER);
      server.send(200, "text/plain", "OK");
    }
