...
//********************************               
  ...
  if((horaRtc==0) && (minRtc==0))   //zera o index diariamente a 0:00 h
    indice=0;
      ...
      //**************bilheteSD**********************
      // (" ANO MES DIA HORA:MIN DSemana TRTC  TBMP PBMP ABMP  Tfora Tagua TcoolerF TcoolerQ Comb RPM Nagua");
      //*********************************************
  
   bilheteSD = (" "+String(anoRtc)+"-"+String(mesRtc)+"-"+String(diaRtc)+"-"+" "+String(horaRtc)+":"+minutos+" "+String(semana)+" "+String(tempRtc)+" "+String(tempBmp280)+" "+String(presBmp280)+" "
                   +String(altiBmp280)+" "+String(temp18b20_0)+" "+String(temp18b20_1)+" "+String(temp18b20_2)+" "+String(temp18b20_3)+" "+String(combAnt)+" "+String(RPM)+" "+agua);
   ...
   if((minAnt!=minRtc) && motorLIGADO)
   {
      minAnt=minRtc;
      logSDminuto( diaRtc, mesRtc, anoRtc, horaRtc, minRtc);
   }
    ...
    if(clientWsNum>0)
    {
      ...
      bilheteWs=socketSend;
      webSocket.broadcastTXT(bilheteWs);          
    }
